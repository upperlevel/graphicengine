...
import static xyz.upperlevel.graphicengine.launcher.api.GameNeed.*;
...
public class GameIdentity {
    ...
    public static GameIdentity load(Map<String, Object> data) {
        if (!data.containsKey(GAME_GROUPID.getOnly()))
            throw new IllegalArgumentException("GroupId doesn't found");
        String id = (String) data.get(GAME_ID.getOnly());
        if (!data.containsKey(GAME_ID.getOnly()))
            throw new IllegalArgumentException("Id doesn't  found");
        if (!data.containsKey(GAME_VERSION.getOnly()))
            throw new IllegalArgumentException("Version doesn't found");
        String displayName = data.containsKey(GAME_DISPLAYNAME.getOnly()) ? (String) data.get(GAME_DISPLAYNAME.getOnly()) : id;
        return new GameIdentity(
                (String) data.get(GAME_GROUPID.getOnly()),
                (String) data.get(GAME_ID.getOnly()),
                (String) data.get(GAME_VERSION.getOnly()),
                displayName
        );
    }
